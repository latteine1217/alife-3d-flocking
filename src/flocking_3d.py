...
class Flocking3D:
    ...
    def __init__(self, N: int, params: FlockingParams):
        ...
        # 邊界模式設定（優先使用 boundary_mode，向後相容 use_pbc）
        if params.boundary_mode == "reflective":
            ...
            self.boundary_mode = 0  # PBC（預設）
        else:
            self.boundary_mode = 0  # Fallback
        ...
        # 診斷用累加器
        self.diag = ti.field(ti.f32, 7)  # [sum_vx, vy, vz, speed, sum_x, y, z]
        ...
        print(
            f"[Flocking3D] N={N}, Boundary={boundary_str}, beta={params.beta}, eta={params.eta}"
        )
    ...
    def initialize(self, box_size: float = None, v_scale: float = 0.1, seed: int = 0):
        ...
        x_init = rng.uniform(-box_size, box_size, (self.N, 3)).astype(np.float32)
        v_init = rng.uniform(-v_scale, v_scale, (self.N, 3)).astype(np.float32)
    ...
    @ti.func
    def pbc_dist(self, xi: ti.template(), xj: ti.template()) -> ti.math.vec3:
    ...
    def compute_forces(self):
        """計算所有力（Morse + Alignment + Soft-sphere Repulsion）
        ...
        # 軟球排斥參數
        min_distance = 0.8  # 最小距離（agents 不會比這更靠近）
        repulsion_strength = 10.0  # 排斥力強度
        ...
        for i in self.x:
            ...
            for j in range(self.N):
                ...
                # === 軟球排斥力（新增）===
                # 當距離 < min_distance 時，施加強力排斥
                if r < min_distance:
                    # 排斥力 = k * (min_distance - r) / r^2
                    overlap = min_distance - r
                    repulsion_coeff = repulsion_strength * overlap / (r2 + 1e-6)
                    force -= repulsion_coeff * rij * inv_r

                # === Morse force（僅在 rc 範圍內）===
                if r2 <= rc2:
    ...
    def verlet_step1(self, dt: ti.f32):
        ...
        for i in self.x:
            # 半步速度：a = F / m（修正：正確使用質量）
            mass = self.p[9]  # 在基礎類別中，質量是全局參數
            a = self.f[i] / mass
    ...
    def verlet_step2(self, dt: ti.f32):
        ...
        for i in self.v:
            ...
            # Vicsek noise: 3D 球面隨機旋轉（Axis-Angle 方法）
            if eta > 0.0:
                ...
                if speed > 1e-6:
                    ...
                    # Random 1: 旋轉角度 [-eta, +eta]
                    state = self.xorshift32(state)
                    rand1 = self.rand_uniform(state)
                    noise_angle = (rand1 - 0.5) * 2.0 * eta

                    # Random 2-3: 隨機旋轉軸（球面均勻分布）
                    # 使用 Marsaglia (1972) 方法生成均勻球面向量
                    state = self.xorshift32(state)
                    ...
                    # 拒絕取樣：若超出單位圓，用速度方向的正交向量
                    if s < 1.0 and s > 1e-6:
                        # Marsaglia 方法：(x, y, z) = (2u√(1-s), 2v√(1-s), 1-2s)
    ...
    def _accumulate_diag(self):
        ...
        for i in self.v:
            ...
            ti.atomic_add(self.diag[2], v[2])
            ti.atomic_add(self.diag[3], speed)
            ti.atomic_add(self.diag[4], x[0])
            ti.atomic_add(self.diag[5], x[1])
            ti.atomic_add(self.diag[6], x[2])

    @ti.kernel
    def _accumulate_rg(self, cx: ti.f32, cy: ti.f32, cz: ti.f32):
        """累加 Rg（第二次掃描）"""
...
# 向後相容性別名
# ============================================================================
OptimizedFlockingV2 = Flocking3D
